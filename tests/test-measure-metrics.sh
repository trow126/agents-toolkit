...
assert_metric "実 repo: Codex skill は4件(新規behavior skill純増なし)" "$out" "codex_skills" "4"
...
assert_metric "実 repo: bypass permission mode lockout" "$out" "bypass_lockout_ok" "yes"
...
assert_metric "実 repo: sandbox Bash auto-allow disabled" "$out" "sandbox_auto_allow_bash" "no"
