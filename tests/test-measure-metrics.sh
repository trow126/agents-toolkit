...
assert_metric "実 repo: routing 表は14行" "$out" "routing_rows" "14"
...
assert_metric "実 repo: Claude effective skill は18件" "$out" "claude_skills" "18"
assert_metric "実 repo: Codex effective skill は19件" "$out" "codex_skills" "19"
assert_metric "実 repo: Codex custom agent は2件" "$out" "codex_custom_agents" "2"
...
assert_metric "実 repo: 要素別 inventory 行数" "$out" "inventory_audited_elements" "187"
assert_metric "実 repo: review/progress/retrospective active unique path" "$out" "review_progress_retrospective_mechanisms" "5"
...
assert_metric "実 repo: SessionStart の systemMessage なし（Phase 7）" "$out" "session_start_system_message_max_bytes" "0"
assert_metric "実 repo: PostCompact の systemMessage なし（Phase 7）" "$out" "post_compact_system_message_max_bytes" "0"
assert_metric "実 repo: UserPromptSubmit 出力なし（D3②）" "$out" "user_prompt_submit_injection_max_bytes" "0"
