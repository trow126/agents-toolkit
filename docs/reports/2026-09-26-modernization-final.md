...
- Codex: `~/.codex/config.toml` の既定（gpt-6-astra/medium）と `[agents]`（gpt-6-sol/high、P8-Q4 で変更）は routing 表と一致している。profile の hook の trust は `config.toml` にある。trust を移す前の file は `~/.local/state/agents-toolkit/backup/codex-config.toml.before-trust-move` に退避してある。
