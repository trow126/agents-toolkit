...
| 承認者 | 要件所有者(T-row) | 要件所有者(本 v8 再レビューで追認予定。設計判断の経緯は下記) |
| 承認日 / 発言 | 2026-07-23 「例外で良い」(ATK-010 の指摘に対する回答) | 2026-07-24 記録(v2 以降 CLAUDE.md に「新しい継続的な知見は native auto memory に保存する」と明記し、5 回のレビューを通過した設計) |
