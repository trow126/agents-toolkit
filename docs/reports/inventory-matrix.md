...
| hook:prompt-submit-hook | `-` → `claude/hooks/prompt-submit-hook.sh` | — | silenced UserPromptSubmit hook kept until the managed registration is removed (owner decision D3②) | high | partial / core-contract and CLAUDE.md overlap | none (no output) | low / low | managed registration + silent-output fixtures | yes / n/a | silence (D3②); unregister in managed policy |
...
| validation:test-prompt-submit-hook | `-` → `tests/test-prompt-submit-hook.sh` | — | verify the UserPromptSubmit hook stays silent and exits 0 for every input (D3②) | high | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
