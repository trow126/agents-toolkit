...
|---|---|---|---|---|---|---|---|---|---|---|
| agent:ai-engineer | `claude/agents/ai-engineer.md` → `-` | — | AI/ML production implementation | high | none / none | none | low / medium | frontmatter/schema + task review | partial / lint/tests supplement | delete (owner decision D6) |
| agent:blockchain-security-auditor | `claude/agents/blockchain-security-auditor.md` → `-` | review | independent smart-contract security audit | high | none / domain-specific; not generic reviewer | none | over-reporting possible / high | read-only isolation + review output | no / SAST supplements only | delete (owner decision D6) |
| agent:code-reviewer | `claude/agents/code-reviewer.md` → `-` | review-progress-retrospective | independent code/security review | high | none / security-reviewer merged here | none | duplicate findings / medium | read-only tool restriction + review tests | yes / lint/SAST supplements | delete (owner decision D6) |
| agent:data-engineer | `claude/agents/data-engineer.md` → `-` | — | data pipeline design and implementation | high | none / none | none | low / medium | frontmatter/schema + task review | partial / data validation supplements | delete (owner decision D6) |
| agent:deep-reasoner | `claude/agents/deep-reasoner.md` → `-` | routing | high-risk architecture/root-cause reasoning | conditional | none / built-in general reasoning overlaps partially | none | overuse raises cost / medium | routing contract and owner review | no / no | delete (owner decision D6) |
...
| agent:model-qa-specialist | `claude/agents/model-qa-specialist.md` → `-` | review | independent ML quality/fairness audit | high | none / none | none | false positives from incomplete evidence / high | read-only audit output | partial / evaluation scripts supplement | delete (owner decision D6) |
| agent:plan-reviewer-completeness | `claude/agents/plan-reviewer-completeness.md` → `-` | review-progress-retrospective | plan completeness review | medium | none / two sibling plan reviewers | none | triple-review cost / low | merged-agent contract | yes / no | delete (owner decision D6) |
| agent:plan-reviewer-critic | `claude/agents/plan-reviewer-critic.md` → `-` | review-progress-retrospective | plan critique | medium | none / two sibling plan reviewers | none | triple-review cost / low | merged-agent contract | yes / no | delete (owner decision D6) |
| agent:plan-reviewer-feasibility | `claude/agents/plan-reviewer-feasibility.md` → `-` | review-progress-retrospective | plan feasibility review | medium | none / two sibling plan reviewers | none | triple-review cost / low | merged-agent contract | yes / no | delete (owner decision D6) |
...
| agent:security-reviewer | `claude/agents/security-reviewer.md` → `-` | review-progress-retrospective | generic security review | medium | code-reviewer security checklist / code-reviewer | none | duplicate findings / medium | merged-agent contract | yes / SAST supplements | delete (owner decision D6) |
| agent:solidity-engineer | `claude/agents/solidity-engineer.md` → `-` | — | secure Solidity implementation | high | none / none | none | low / high | frontmatter/schema + domain tests | partial / forge/slither supplement | delete (owner decision D6) |
| agent:sre | `claude/agents/sre.md` → `-` | — | reliability/observability implementation | high | none / none | none | low / high | frontmatter/schema + task tests | partial / monitoring tests supplement | delete (owner decision D6) |
...
| agent:codex-reviewer | `codex/agents/reviewer.toml` → `-` | review-progress-retrospective | independent code/security review on Sol | high | codex review overlaps partially / narrow reusable review role | none | duplicate findings / medium | TOML schema + runtime session metadata | no / lint/SAST supplements | delete (owner decision P8-Q3) |
...
| agent:codex-deep-reasoner | `codex/agents/deep_reasoner.toml` → `-` | routing | high-risk architecture and root-cause analysis on Sol xhigh | conditional | built-in general reasoning overlaps partially / escalation-only role | none | overuse raises cost / medium | TOML schema + runtime session metadata | no / no | delete (owner decision P8-Q3) |
...
| claude-skill:break-consensus | `-` → `shared/skills/claude-code/break-consensus/SKILL.md` | innovation | manual innovation exploration; opt-in --cross adds a Codex and a read-only Claude worker (D7) | required | none / none | manual-only | inappropriate use / low | manual-only flag + novelty audit | partial / no | add |
...
| claude-skill:gh:issue | `claude/skills/gh:issue/SKILL.md` → `shared/skills/gh-issue/SKILL.md` | — | issue workflow | high | none / none | manual-only | external action confusion / medium | contract tests + ask rules | yes / gh CLI helper | rename/keep |
...
| claude-skill:gh:pr | `claude/skills/gh:pr/SKILL.md` → `shared/skills/gh-pr/SKILL.md` | — | pull-request workflow | high | none / none | manual-only | external side effect / high | ask rules + skill schema | yes / gh CLI | rename/keep |
| claude-skill:gh:review | `claude/skills/gh:review/SKILL.md` → `shared/skills/gh-review/SKILL.md` | review-progress-retrospective | PR review workflow | high | none / pr-review overlap limited to entrypoint | manual-only | duplicate review / low | skill schema + review contract | yes / deterministic checks first | rename/keep |
| claude-skill:gh:start | `claude/skills/gh:start/SKILL.md` → `shared/skills/gh-start/SKILL.md` | routing | issue start/implementation owner workflow | high | none / project-orchestrator/fast-worker removed | manual-only | handoff churn / medium | test-gh-start-contract | yes / gh-issue-fetch | rename/simplify |
...
| claude-skill:plan-review | `claude/skills/plan-review/SKILL.md` → `-` | review-progress-retrospective | invoke independent plan review | high | none / plan-reviewer is executor not duplicate | manual-only | duplicate review / low | skill/agent contract | yes / deterministic checks first | delete (owner decision D6) |
| claude-skill:pr-review | `claude/skills/pr-review/SKILL.md` → `-` | review-progress-retrospective | PR review process | high | none / gh-review entrypoint overlap controlled | manual-only | duplicate review / low | skill schema | yes / deterministic checks first | delete (merged into gh-pr --review-comment) |
...
| resource:claude-gh-issue-references | `-` → `shared/skills/gh-issue/references` | — | package-local Issue workflow reference | high | none / canonical shared reference | on-demand | stale link / high | validator + runtime audit | n/a / yes | add |
| resource:claude-gh-pr-references | `-` → `shared/skills/gh-pr/references` | — | package-local PR workflow reference | high | none / canonical shared reference | on-demand | stale link / high | validator + runtime audit | n/a / yes | add |
| resource:claude-gh-review-references | `-` → `shared/skills/gh-review/references` | — | package-local review workflow reference | high | none / canonical shared reference | on-demand | stale link / high | validator + runtime audit | n/a / yes | add |
| resource:claude-gh-start-references | `-` → `shared/skills/gh-start/references` | — | package-local implementation workflow reference | high | none / canonical shared reference | on-demand | stale link / high | validator + runtime audit | n/a / yes | add |
...
| helper:emit-system-message | `-` → `-` | runtime-state | emit JSON-safe bounded hook systemMessage output | high | none / none | SessionStart/PostCompact | truncation at byte bound / medium | test-hook-context + hook metrics | n/a / yes | delete (Phase 7: no remaining systemMessage hook) |
...
| helper:codex-delegate | `-` → `claude/bin/codex-delegate` | delegation | launch one contracted Codex delegation with codex exec and the toolkit-implementer profile | high | none / none | on demand | low / high | test-codex-delegate | n/a / yes | add |
| helper:codex-delegate-preflight | `-` → `claude/bin/codex-delegate-preflight` | delegation | reject a contract that violates its schema, route catalog, effort, or profile preconditions | high | none / none | on demand | low / high | test-codex-delegate | n/a / yes | add |
| helper:verify-delegation | `-` → `claude/bin/verify-delegation` | delegation | post-delegation gate over the diff since baseline; writes evidence JSON | high | none / none | on demand | low / high | test-codex-delegate | n/a / yes | add |
| helper:delegation-evidence-check | `-` → `claude/bin/delegation-evidence-check` | delegation | gh-finish completion gate requiring passing evidence that matches HEAD and diff | high | none / none | on demand | low / high | test-codex-delegate | n/a / yes | add |
| helper:delegation-common | `-` → `claude/bin/delegation_common.py` | delegation | shared schema, baseline, scope, and state helpers for the delegation commands | high | none / none | on demand | low / high | test-codex-delegate | n/a / yes | add |
| helper:codex-profile-trust | `-` → `scripts/codex-profile-trust.py` | delegation | move hook and project trust that Codex writes into a toolkit profile to CODEX_HOME/config.toml and restore the profile | high | none / none | on demand | low / medium | test-codex-profile-trust | n/a / yes | add |
| helper:break-consensus-cross | `-` → `claude/bin/break-consensus-cross` | innovation | run the Codex worker of /break-consensus --cross and check both workers' outputs, route, sandbox, and git status | required | none / none | on demand | low / medium | test-break-consensus-cross | n/a / yes | add |
...
| hook:post-compact-hook | `claude/hooks/post-compact-hook.sh` → `-` | — | restore bounded repository orientation after compaction | high | partial / rules/CI where noted | PostCompact systemMessage | low / low | JSON + byte-bound test | yes / script | delete (Phase 7: systemMessage reaches only the user) |
...
| hook:prompt-submit-hook | `-` → `-` | — | silenced UserPromptSubmit hook kept until the managed registration is removed (owner decision D3②) | high | partial / core-contract and CLAUDE.md overlap | none (no output) | low / low | managed registration + silent-output fixtures | yes / n/a | delete (owner decision D3②: registration removed in Phase 7) |
| hook:pr-review-hook | `claude/hooks/pr-review-hook.sh` → `claude/hooks/pr-review-hook.sh` | review-progress-retrospective | after gh pr create, point Claude to /code-review and gh-pr --review-comment via additionalContext | high | partial / rules/CI where noted | PostToolUse Bash | false positives / low | tests/test-pr-review-hook.sh | yes / script | keep/fix (pr-review merged into gh-pr --review-comment) |
...
| hook:session-init-hook | `claude/hooks/session-init-hook.sh` → `-` | — | inject bounded repository orientation | high | partial / rules/CI where noted | SessionStart systemMessage | low / low | JSON + byte-bound test | yes / script | delete (Phase 7: systemMessage reaches only the user; the model gets gitStatus) |
| hook:slack-notify-hook | `claude/hooks/slack-notify-hook.sh` → `claude/hooks/slack-notify-hook.sh` | — | send explicit lifecycle notifications to Slack; AGENTS_TOOLKIT_SLACK_NOTIFY=off opts out | high | partial / rules/CI where noted | event-only | network/config failure / low | tests/test-slack-notify-hook.sh + registration | yes / script | keep |
...
| id | before → after | tags | purpose | needed | built-in / overlap | context | false positive / failure | verification | low-cost / deterministic | disposition |
|---|---|---|---|---|---|---|---|---|---|---|
| codex-reference:python-quality | `-` → `-` | lazy | Python quality guidance loaded only when needed | high | none / formerly inline in AGENTS.md | on-demand | low / low | sync-shared-rules --check | yes / lint supplements | delete (python-guidelines read directly) |
...
| mechanism:codex-implementer-profile | `-` → `codex/profiles/toolkit-implementer.config.toml` | — | Codex profile for delegated implementation: developer instructions, no subagents/skills/memories, apply_patch lint hook | high | none / none | per delegation | low / medium | codex -p toolkit-implementer debug prompt-input; test-codex-delegate | n/a / yes | add |
| mechanism:codex-divergent-profile | `-` → `codex/profiles/toolkit-divergent.config.toml` | — | Codex profile for the /break-consensus --cross worker: developer instructions, no skill list, subagents, or memories | required | none / none | per cross run | low / low | codex -p toolkit-divergent debug prompt-input; audit-context-runtime | n/a / yes | add |
...
| mechanism:managed-permissions | `-` → `claude/managed-settings.json` | — | non-overridable owner-selected bypassPermissions default | high | managed settings official feature / deny enforced under bypass; allow/ask skipped | session configuration | deny only; no allow/ask or sandbox enforcement / high | policy checker + installer + live doctor | n/a / yes | add managed policy |
...
| mechanism:managed-env-pins | `-` → `claude/managed-settings.json` | — | pin alias resolution (fable, opus, sonnet, haiku) with managed env; promotions are explicit | high | none / none | none | low / medium | check-model-routing; check-managed-policy; discovery alias_resolution | n/a / yes | add |
| mechanism:user-settings | `claude/settings.json` → `-` | — | non-security user preferences, model alias and plugin choices | high | none / security policy removed to managed scope | session configuration | plugin/config drift / medium | JSON + policy checker | n/a / yes | delete (owner decision D5: live ~/.claude/settings.json is canonical) |
...
| mechanism:model-pin-scanner | `-` → `scripts/lib/scan-model-pins.py` | — | parse supported YAML/JSON/TOML model routing syntax fail-closed; --names scans distributed files for model names outside routing targets | high | none / none | on demand/event | low / high | validator and metric negative fixtures | n/a / yes | add/keep |
| mechanism:model-routing-checker | `-` → `scripts/lib/check-model-routing.py` | — | check the routing table schema, target values, agent-file membership, and env pins | high | none / model-pin-scanner (syntax only) | CI | low / high | test-model-routing | n/a / yes | add |
| mechanism:runtime-discovery | `-` → `scripts/discover-runtime.sh` | — | read-only runtime/model discovery against the routing table with snapshot diff and Level suggestions | high | none / check-runtime.sh (install-time doctor only) | manual/on change | low / medium | test-discover-runtime | n/a / yes | add |
| helper:runtime-discovery | `-` → `scripts/lib/discover_runtime.py` | runtime-utility | collect Claude/Codex runtime facts, compare with routing table and previous snapshot | high | none / none | manual/on change | low / medium | test-discover-runtime | n/a / yes | add |
...
| mechanism:distributed-markdown-lint | `-` → `scripts/lint-distributed-markdown.sh` | — | run post_edit_lint Markdown checks over manifest-distributed tracked Markdown | high | none / post-edit-lint hook (same checks, edit-time only) | CI | low / low | test-lint-distributed-markdown | n/a / yes | add |
...
| validation:test-hook-context | `-` → `tests/test-hook-context.sh` | — | verify no managed hook injects per-prompt text or systemMessage-only JSON, and zero injection metrics | high | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
...
| validation:test-model-routing | `-` → `tests/test-model-routing.sh` | — | verify routing table checks, model-name scan, env pins, and the promote/revert property on fixtures | high | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
| validation:test-discover-runtime | `-` → `tests/test-discover-runtime.sh` | — | verify every discovery FAIL/WARN condition, §2.4 drift detection, and snapshot rotation on fixtures | high | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
| validation:test-codex-delegate | `-` → `tests/test-codex-delegate.sh` | — | verify preflight, launcher, gate, evidence check, worktree git-dir state, and report schema with a stub codex | high | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
| validation:test-codex-profile-trust | `-` → `tests/test-codex-profile-trust.sh` | — | verify the trust move, idempotence, re-trust replacement, and refusal on other profile edits | high | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
| validation:test-break-consensus-cross | `-` → `tests/test-break-consensus-cross.sh` | — | verify prepare, codex, collect, finish, and the skill's read-only contract with a stub codex | required | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
| validation:test-skill-pair-drift | `-` → `tests/test-skill-pair-drift.sh` | — | verify the per-runtime skill pairs differ only by reviewed lines and the merged gh skills have one source dir for both runtimes | required | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
| mechanism:skill-trigger-eval | `-` → `scripts/build-skill-trigger-eval.py` | — | build a claude plugin eval suite for skill trigger accuracy from the model-invocable Claude skills | conditional | none / claude plugin eval runs it | on demand | fixture drift / low | test-skill-trigger-eval | n/a / yes | add |
| validation:test-skill-trigger-eval | `-` → `tests/test-skill-trigger-eval.sh` | — | verify the trigger eval suite holds only model-invocable skills and well-formed cases | required | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
...
| validation:test-prompt-submit-hook | `-` → `-` | — | verify the UserPromptSubmit hook stays silent and exits 0 for every input (D3②) | high | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | delete (Phase 7: hook removed) |
| validation:test-pr-review-hook | `-` → `tests/test-pr-review-hook.sh` | — | verify pr-review-hook emits additionalContext only after a successful gh pr create and names active skills | high | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
| validation:test-slack-notify-hook | `-` → `tests/test-slack-notify-hook.sh` | — | verify the AGENTS_TOOLKIT_SLACK_NOTIFY=off opt-out exits at once without scheduling a sender | high | none / none | CI/test-only | fixture drift / low | direct CI execution | n/a / yes | add |
| validation:test-lint-distributed-markdown | `-` → `tests/test-lint-distributed-markdown.sh` | — | verify distributed Markdown lint scope (tracked manifest sources only) and violation reporting | high | none / none | CI/test-only | fixture drift / medium | direct CI execution | n/a / yes | add |
