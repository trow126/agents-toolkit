...
| `claude/settings.local.json` | private overlay | move to real dir（保存のみ） | 2026-07-23 再監査: `~/.claude/settings.local.json` は Claude Code の documented scope ではなく、実 directory へ移しても**読み込まれない**。migration は内容保全のための退避として扱う。非 security preference だけを user/project local へ手動移行し、permission・hook・sandbox は `claude/managed-settings.json` をレビューして OS-managed scope へ導入する。project/local の security key は runtime gate が拒否する（旧 `claude-bypass` launcher は 2026-07-23 に廃止済み・REMOVED）（[claude/README.md](../../claude/README.md) の「マシン固有の設定差分」参照） |
