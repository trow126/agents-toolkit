...
| `claude/CLAUDE.local.md` | private routing | move to XDG config | user-levelの`~/.claude/CLAUDE.local.md`はClaude Codeのdocumented load pathではない。`${XDG_CONFIG_HOME:-$HOME/.config}/agents-toolkit/private-routing.md`へ移し、project-orchestratorが存在時だけ明示参照する。project固有指示は各project rootの`CLAUDE.local.md`へ置く |
