...
- Every Codex launch follows the `gh-codex-drive` workflow verbatim: one contract per sub-Issue with the route from its `codex-route.env`, `~/.claude/bin/codex-delegate` in the background, then the `verify-delegation` gate. Each delegation is a fresh run.
- `/gh-finish --apply` for a delegated sub-Issue requires passing evidence (`delegation-evidence-check`); a missing or stale evidence stops the loop.
