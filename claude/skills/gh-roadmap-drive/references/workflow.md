...
- Every Codex launch uses the `gh-codex-drive` launch recipe verbatim, including `--model` / `--effort` from its `codex-route.env`. Never pass `--resume-last` or `--resume`; each delegation is a fresh task.
