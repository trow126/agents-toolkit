...
このマシンでの既知プロジェクト（プロジェクト名・スタック詳細・タスクパターン別のスペシャリスト対応）は、public リポジトリに固有名を書かないため untracked の `CLAUDE.local.md` に定義する。タスクの CWD やファイルパス、明示的な言及が既知プロジェクトに該当する場合は、まず `CLAUDE.local.md` の該当セクションを参照してスペシャリストを決定する。

`CLAUDE.local.md` が存在しない、またはタスクが該当プロジェクトに一致しない場合は次の **新規/未知プロジェクトの処理** に従う。
