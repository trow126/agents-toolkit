...
- **`pkill -f` は自分を実行中のshellにも一致する**: パターン文字列を含む `bash -c` 自体が kill され exit 144 で中断する。先頭文字を括弧で囲み `pkill -f "[k]aizen_keiba.loop"` とする（`pgrep -f` も同様）(理由: 4 session で独立に再発)

### エージェント設定の注意点

- **共有ルールは原則ベースに保つ**: 細則を列挙せず、モデルの判断に任せる。共有正本は Codex も消費するため、Codex 側で遵守低下を観測したら Codex 専用の補足を `codex/` 側に足す（正本を再肥大化させない）
