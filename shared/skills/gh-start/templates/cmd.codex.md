...
> **原則**: GitHub Issue = SSOT、checkpointファイル = 進捗追跡、逐次実行 = 確実性
...
1. **checkpoint確認**: `.codex/checkpoints/issue_{N}_checkpoint.md` を確認
   - checkpoint存在 → レジュームを提案（残タスクのみ実行）
   - checkpoint不在 → 新規実行

2. **Issue取得**: `~/.claude/bin/gh-issue-fetch.sh ${ISSUE_NUMBER}` を実行（Claude Code側の共有helperを絶対パスで直接呼び出す。Codex固有の複製は持たない）
...
```bash
# 実行コマンド
~/.claude/bin/gh-issue-fetch.sh 42
```

**成功条件**: exit 0 + 有効なJSON + state == "open"

5. **checkpoint初期化**: `.codex/checkpoints/issue_{N}_checkpoint.md` にタスク一覧・状態を保存
...
1. **タスク一覧構築**:
   - pending タスクのみ抽出
   - checkpointファイルに各タスクの状態(pending/in_progress/completed)を記録

2. **逐次実行** (タスクごとに):
   - checkpointファイルを `in_progress` に更新
   - **現在の owner（このセッション）が自分で実装・テスト・修正まで完遂する**（既定。委譲しない）
   - 完了後 → checkpointファイルを `completed` に更新
...
**委譲の条件（例外）**:
タスク数・ステップ数は委譲理由にならない。以下のいずれかに**明示的に該当する場合のみ** 独立コンテキストの subagent を使い、該当理由と委譲先を checkpoint に 1 行記録する:

- **context isolation**: 大量の read-only 探索・長大ファイル読み込みが main context を汚染する → `codex exec` に read-only の調査タスクとして切り出す
- **specialist expertise**: 該当ドメインの高リスク・専門作業（AGENTS.md のドメインルーティング該当時のみ）
- **independent verification**: 高リスク変更の独立監査が必要 → `codex review`（read-only。実装はさせない）
- **useful parallelism**: 相互依存のない独立検証を並行比較する場合のみ

委譲した場合も、結果の統合・最終検証・commit は owner が行う。委譲した呼び出しがエラーを返した場合、ユーザーに報告して指示を待つ。
...
3. **自己検証** (`~/.agents/rules/learnings.md` ゲート):
   ...
   - テストが存在する場合、`uv run pytest` が通るか（sandbox制約がある環境では `~/.claude/bin/uvw run pytest` 等のcache-path互換wrapperを使う）
...
1. **進捗更新**: `~/.claude/bin/gh-progress-sync.sh --json '{...}'` で GitHub Issue に進捗コメント投稿
...
```bash
# 進捗同期
~/.claude/bin/gh-progress-sync.sh --json '{"issue": 42, "completed": [1,2], "total": 5, "task_name": "Task name"}'

# タスク個別チェック
~/.claude/bin/gh-progress-sync.sh --check-task 42 "Implement login button"
```
...
7. [Sync] GitHub更新完了
8. [Done] 全タスクが完了しました。変更内容を確認してください。
```
...
- `jq` (JSONパース)
- `python3` (パーサースクリプト)
- ローカルcheckpoint: `.codex/checkpoints/issue_{N}_checkpoint.md`
