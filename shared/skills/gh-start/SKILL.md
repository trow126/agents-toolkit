---
name: gh-start
description: Starts or continues implementing a GitHub Issue with local edits and tests; --commit and --sync are separate explicit modes. Use when the user asks to work on an Issue in this session. Not for gh-pr or gh-review.
argument-hint: "<issue-number> [--commit|--sync]"
---

# gh-start

Invoked as `/gh-start` in Claude Code and `$gh-start` in Codex.

**Stop** on `main`/`master`, conflicting dirty changes, missing requirements, or failed tests that cannot be resolved in scope. **Done** when the local changes and test results are reported with the exact operations not performed.
