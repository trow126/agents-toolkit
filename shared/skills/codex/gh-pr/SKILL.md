...
> **原則**: PR作成に特化。セルフレビューはこのスキルの Phase 5 内で明示的に実行する。レビュー結果は PR コメントに投稿するだけで、修正は開始しない。
...
- **`gh pr create` は1回のみ実行**: 失敗時はコマンドを表示してユーザーに手動委譲（多重実行防止）
- **レビュー後の即時修正禁止**: `codex review` のレビュー結果を受け取って、そのままファイル編集・追加コミット・追加 push を開始しない
...
3. **別コンテキストレビュー**: `codex review --uncommitted`（プッシュ済みの対象コミット全体をレビューする場合は `codex review --base <base-branch>`）を実行し、独立コンテキストでのレビュー結果を取得
   - 観点: バグ、セキュリティ、パフォーマンス、可読性、テスト漏れ
   - 出力形式は `pr-review` スキル（skills/pr-review/SKILL.md）の `## Automated Code Review` テンプレートに従って整形する
   - 指摘がない場合も、要修正「なし」としてコメント本文を生成する
   - `codex review` の実行はレビューコメント本文の生成だけに使う。結果を受けてこのステップ内で修正実装を行わない
