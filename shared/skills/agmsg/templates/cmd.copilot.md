...
Otherwise, run: `~/.agents/skills/agmsg/scripts/whoami.sh "$(pwd)" copilot`
...
→ Show the user the available teams from the output, then:
  ...
  3. **You MUST use join.sh** — run: `~/.agents/skills/agmsg/scripts/join.sh <team> <agent_name> copilot "$(pwd)"`
  ...
  5. **REQUIRED — Do NOT skip this step.** Ask the user to pick a delivery mode using exactly this prompt:
     ...
     - Map the chosen number to a mode (`1`→`turn`, `2`→`off`) and run:
       `~/.agents/skills/agmsg/scripts/delivery.sh set <mode> copilot "$(pwd)"`
...
→ No exact registration exists for this project, but there are same-type agent names registered elsewhere.
  ...
  4. Run: `~/.agents/skills/agmsg/scripts/join.sh <team> <agent_name> copilot "$(pwd)"`
...
2. Run `~/.agents/skills/agmsg/scripts/identities.sh "$(pwd)" copilot` to see whether the role is already registered for this (project, type).
3. If the name does not appear in the output, join under the existing team. For a single team, run `~/.agents/skills/agmsg/scripts/join.sh <team> <name> copilot "$(pwd)"`. For multiple teams, ask the user which team to join the new role into.
...
2. Run `~/.agents/skills/agmsg/scripts/reset.sh "$(pwd)" copilot <name>` to remove that role's registration.
3. If the session's active FROM was `<name>`, clear that state.
4. Tell the user: "Dropped role `<name>` from this project."

If argument is "mode" (no further args):
1. Run: `~/.agents/skills/agmsg/scripts/delivery.sh status copilot "$(pwd)"`
2. Show the output to the user.
...
2. Run: `~/.agents/skills/agmsg/scripts/delivery.sh set <mode> copilot "$(pwd)"`

If argument is "hook on" (legacy alias):
1. Run: `~/.agents/skills/agmsg/scripts/delivery.sh set turn copilot "$(pwd)"`
2. Tell the user: "Delivery mode set to 'turn' (legacy hook on behavior)."

If argument is "hook off" (legacy alias):
1. Run: `~/.agents/skills/agmsg/scripts/delivery.sh set off copilot "$(pwd)"`
2. Tell the user: "Delivery mode set to 'off'."

If argument is "reset":
1. Run: `~/.agents/skills/agmsg/scripts/reset.sh "$(pwd)" copilot`
