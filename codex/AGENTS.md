...
- 速度より品質を優先する。MVP = スコープの最小化であり、品質の最小化ではない。
- 不確実なら推測せず調査を深める。
- 「動けば良い」より「正しく動く」。
...
- 依頼された範囲のみ実装。投機的な機能・汎用化・「将来のための抽象」を作らない（YAGNI）。
- バグ修正に周辺リファクタを混ぜない。
- 完成させる: TODO・モック・スタブを残さない。本番品質のコードのみ。
...
- 空・単一・境界値・無効値・NaN を網羅する。
- 永続化・再読込・実行時更新を伴う変更では、単体テストだけでなく実運用の状態遷移を再現する round-trip テストを作成する（プロジェクト固有の対象列は各リポジトリの AGENTS.md に記載）。
